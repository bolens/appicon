...
| Surface | Notes |
|---------|--------|
| Config / cache / packs | Honor absolute `XDG_*` paths when set; otherwise use OS user config/cache dirs. Packs default to `~/.local/share` on Linux, Application Support on macOS, and `%LOCALAPPDATA%` on Windows |
