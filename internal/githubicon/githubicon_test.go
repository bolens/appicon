...
func TestDownloadRejectsHTTP(t *testing.T) {
	...
	_, err := c.Lookup(context.Background(), "bolens", githubicon.Options{Token: "ghp_test"})
	if err == nil {
		t.Fatal("expected http avatar URL rejection")
	}
}

func TestOverrideRejectsUnrelatedAvatarHost(t *testing.T) {
	t.Setenv("XDG_CACHE_HOME", t.TempDir())
	srv := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`{"avatar_url":"https://evil.example/avatar.png"}`))
	}))
	defer srv.Close()
	c := githubicon.New()
	c.HTTP = srv.Client()
	...
	c.BaseURL = srv.URL
	_, err := c.Lookup(context.Background(), "bolens", githubicon.Options{Token: "ghp_test"})
	if !errors.Is(err, githubicon.ErrNotFound) {
		t.Fatalf("err=%v want ErrNotFound", err)
	}
}

func TestOverrideRejectsUnsafeRedirects(t *testing.T) {
	for _, location := range []string{
		...
		t.Run(location, func(t *testing.T) {
			...
			c.HTTP = srv.Client()
			c.APIBaseURL = srv.URL
			_, err := c.Lookup(context.Background(), "org/repo/icon.svg", githubicon.Options{Token: "ghp_test"})
			if !errors.Is(err, githubicon.ErrNotFound) {
...
func TestOverridePreservesRedirectPolicy(t *testing.T) {
	t.Setenv("XDG_CACHE_HOME", t.TempDir())
	called := false
	srv := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
