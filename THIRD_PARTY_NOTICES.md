...
## Icons and binary dependencies

Downloaded icon packs and cached provider assets retain their own licenses and
trademark restrictions. The application's MIT grant does not grant rights to all
resolved icons. Keep each pack's license, attribution, and per-icon exceptions
with redistributed packs or exported assets. `docs/sources.md` and `docs/packs.md`
describe the providers. The dependency notices below cover the Go module inputs,
not externally downloaded icon collections.
